...
public enum SensorInfoType { DRIVER_SENSOR =5, SHOOTER_SENSOR = 4 };
...
public enum ConfigRayCastType
    ...
    public struct ConfigRayInput
    {
        ...
        /// </summary>
        public ConfigRayCastType CastType;
        ...
        public SensorInfoType infoType;
    /// <summary>
    ...
    {
        return (int)infoType * Angles.Count;
        ...
        public (Vector3 StartPositionWorld, Vector3 EndPositionWorld) RayExtents(int rayIndex)
        {
            ...
            if (CastType == ConfigRayCastType.Cast3D)
            {
    ...
    public class ConfigRayOutput
    {
        ...
        public struct RayOutput
        {
            ...
            internal SensorInfo si;
            internal SensorInfoType infoType;
        ...
        public void ToFloatArray( int rayIndex, float[] buffer)
            {
            var bufferOffset = ((int)infoType) * rayIndex;
            if (HasHit)
                buffer[bufferOffset] = HitFraction;
            else
                buffer[bufferOffset] = HitFraction;
                ...
                switch (infoType)
                {
                    ...
                    case SensorInfoType.DRIVER_SENSOR:
                        buffer[bufferOffset + 1] = si.moveable ? 1f : 0f;
                        buffer[bufferOffset + 2] = si.IsEnemy(0) ? 1f : 0f;
                        buffer[bufferOffset + 3] = si.isFlag ? 1f : 0f;
                        buffer[bufferOffset + 4] = si.health/100f;
                        return;
...
/// <summary>
/// Debug information for the raycast hits. This is used by the ConfigRaySensorComponent.
/// </summary>
/// 
public class DebugDisplayInfo
    ...
    [System.Serializable]
        public struct RayInfo
        {
            ...
            public float castRadius;
            public ConfigRayOutput.RayOutput rayOutput;
    ...
    /// </summary>
    public class ConfigRaySensor : ISensor
    {
        float[] m_Observations;
        int[] m_Shape;
        string m_Name;

        ConfigRayInput m_ConfigRayInput;

        DebugDisplayInfo m_DebugDisplayInfo;
         public static int rayDataSize = 5;
    ...
    internal DebugDisplayInfo debugDisplayInfo
        ...
        /// <summary>
        /// Creates the ConfigRaySensor.
        /// </summary>
        /// <param name="name">The name of the sensor.</param>
        /// <param name="rayInput">The inputs for the sensor.</param>
        public ConfigRaySensor(string name, ConfigRayInput rayInput)
        {
            m_Name = name;
            m_ConfigRayInput = rayInput;
        ...
        internal void SetConfigRayInput(ConfigRayInput rayInput)
        {
            ...
            if (m_ConfigRayInput.OutputSize() != rayInput.OutputSize())
            {
                Debug.Log(
        ...
        public int Write(ObservationWriter writer)
        {
                ...
                var numRays = m_ConfigRayInput.Angles.Count;
                //var numDetectableTags = m_ConfigRayInput.DetectableTags.Count;
                ...
                for (var rayIndex = 0; rayIndex < numRays; rayIndex++)
                {
                    ...
                    var rayOutput = PerceiveSingleRay(m_ConfigRayInput, rayIndex, out debugRay);
                    ...
                    rayOutput.ToFloatArray(rayIndex, m_Observations);
                }
               // Debug.Log("obs" + m_Observations);
        ...
        public static ConfigRayOutput Perceive(ConfigRayInput input)
        {
            ConfigRayOutput output = new ConfigRayOutput();
            output.RayOutputs = new ConfigRayOutput.RayOutput[input.Angles.Count];
        ...
        /// <summary>
        /// Evaluate the raycast results of a single ray from the ConfigRayInput.
        /// </summary>
        /// <param name="input"></param>
        /// <param name="rayIndex"></param>
        /// <param name="debugRayOut"></param>
        /// <returns></returns>
        internal static ConfigRayOutput.RayOutput PerceiveSingleRay(
            ConfigRayInput input,
            int rayIndex,
        ...
        )
        {
            ...
            SensorInfo si = null;
            if (input.CastType == ConfigRayCastType.Cast3D)
            {
                RaycastHit rayHit;
            ...
            var rayOutput = new ConfigRayOutput.RayOutput
            {
                HasHit = castHit,
            ...
            if (castHit)
            {
            si = hitObject.GetComponent<SensorInfo>();
            if (si != null)
            {
                rayOutput.HitTaggedObject = true;
                //rayOutput.HitTagIndex = 0;
                rayOutput.si = si;
                rayOutput.infoType = input.infoType;
            }
